...
(function(){
  ...
  async function openAssignWO(){
    const input=document.querySelector('#inputPage'),host=input?.querySelector('.tool-list');
    if(!host)return;
    host.innerHTML='<div class="empty"><p>🔄 Membaca teknisi dan order OPEN dari Google Sheet...</p></div>';
    try{
      const u=tgUser();if(!u?.id)throw new Error('Mini App harus dibuka dari Telegram.');
      const r=await fetch('/api/assign-wo?telegram_id='+encodeURIComponent(u.id),{cache:'no-store'});
      const d=await r.json().catch(()=>({}));if(!r.ok||!d.ok)throw new Error(d.message||`HTTP ${r.status}`);
      renderAssignWO(d);
    }catch(e){host.innerHTML=`<div class="empty"><p>❌ ${esc(e.message)}</p><button class="tool-action" id="assignRetry"><b>COBA LAGI</b><span>↻</span></button></div>`;host.querySelector('#assignRetry')?.addEventListener('click',openAssignWO);}
  }
  ...
  function renderAssignWO(d){
    ...
    const orders=(d.orders||[]).map(o=>`<label class="mini-order" style="display:block;cursor:pointer"><input type="checkbox" class="assign-order" value="${esc(o.service_number)}" style="margin-right:8px"><strong>${esc(o.customer_name||'-')}</strong><small style="line-height:1.65">🌐 ${esc(o.service_number)}<br>🎫 ${esc(o.ticket_id||'MANUAL')}<br>📍 ${esc(o.address||'-')}<br>👤 ${o.assigned_technician?`Sudah: ${esc(o.assigned_technician)}`:'Belum di-assign'}</small></label>`).join('');
    host.innerHTML=`<article class="tool-card"><strong>ASSIGN WO</strong><small>Pilih teknisi tujuan dan satu atau beberapa order OPEN. Assignment akan ditulis langsung ke Google Sheet.</small><label style="display:block;margin-top:12px;color:#7890aa;font-size:10px">TEKNISI TUJUAN<select id="assignTech" style="width:100%;margin-top:6px;padding:12px;border-radius:12px;background:#0b1b2e;color:#eef6ff;border:1px solid #294562"><option value="">Pilih teknisi...</option>${techOptions}</select></label><div class="search-wrap" style="margin-top:12px"><span>⌕</span><input id="assignSearch" placeholder="Cari INET, nama, alamat..." /></div><div id="assignOrders" class="mini-order-list" style="margin-top:9px;max-height:55vh;overflow:auto">${orders||'<div class="empty"><p>Tidak ada order OPEN.</p></div>'}</div><button class="tool-action" id="assignSubmit"><b>ASSIGN WO</b><span>→</span></button><button class="tool-action" id="assignRefresh"><b>REFRESH SHEET</b><span>↻</span></button></article>`;
    const original=orders;
    const filter=()=>{const q=String(host.querySelector('#assignSearch')?.value||'').toUpperCase();host.querySelectorAll('.assign-order').forEach(cb=>{const card=cb.closest('.mini-order');card.style.display=(!q||card.textContent.toUpperCase().includes(q))?'block':'none';});};
    host.querySelector('#assignSearch')?.addEventListener('input',filter);
    host.querySelector('#assignRefresh')?.addEventListener('click',openAssignWO);
    host.querySelector('#assignSubmit')?.addEventListener('click',async()=>{
      const tech=host.querySelector('#assignTech')?.value;const selected=[...host.querySelectorAll('.assign-order:checked')].map(x=>x.value);if(!tech)return window.showToast?.('Pilih teknisi tujuan');if(!selected.length)return window.showToast?.('Pilih minimal satu INET');
      const btn=host.querySelector('#assignSubmit');btn.disabled=true;btn.querySelector('b').textContent='MEMPROSES...';
      try{const u=tgUser();const r=await fetch('/api/assign-wo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:String(u.id),target_nik:tech,service_numbers:selected})});const x=await r.json().catch(()=>({}));if(!r.ok||!x.ok)throw new Error(x.message||`HTTP ${r.status}`);window.showToast?.(`✅ ${x.assigned.length} WO berhasil di-assign`);openAssignWO();}catch(e){window.showToast?.('❌ '+e.message);btn.disabled=false;btn.querySelector('b').textContent='ASSIGN WO';}
    });
  }
  ...
  function applyRole(role){
    ...
    if(managerRoles.has(role)){
      ...
      host.innerHTML='<article class="tool-card"><strong>ASSIGN WO</strong><small>Pilih teknisi dan order OPEN untuk proses assignment ke Google Sheet.</small><button class="tool-action" id="injokoAssign"><b>ASSIGN WO</b><span>Mulai ›</span></button></article>';
      host.querySelector('#injokoAssign')?.addEventListener('click',openAssignWO);
    }else{
  ...
  async function loadRole(){
    ...
    try{
      const r=await fetch('/api/technician-profile?telegram_id='+encodeURIComponent(user.id),{cache:'no-store'});const d=await r.json().catch(()=>({}));const profile=d?.profile||{};let role=normalizeRole(profile.role);const nik=String(profile.nik||profile.NIK||profile.nik_teknisi||profile.nikTeknisi||'').replace(/\D/g,'');if(nikRoleMap[nik])role=nikRoleMap[nik];
      try{const mr=await fetch('/api/technician-master?telegram_id='+encodeURIComponent(user.id),{cache:'no-store'});if(mr.ok){const md=await mr.json().catch(()=>({}));const masterRole=normalizeRole(md?.role);if(managerRoles.has(masterRole))role=masterRole;}}catch(e){}
      if(nikRoleMap[nik])role=nikRoleMap[nik];applyRole(role||'TECHNICIAN');
